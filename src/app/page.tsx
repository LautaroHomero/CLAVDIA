...
const SCENARIOS: { label: string; text: string }[] = [
  {
    label: "Turno de control (camino feliz)",
    text: "Hola, soy María Gómez, DNI 28.444.123. Quería un turno de control de presión con la Dra. Ruiz la semana que viene a la mañana.",
  },
  {
    label: "Renovación de receta (aprobación)",
    text: "Buenas, necesito que me renueven la receta de Apixabán. Soy Jorge Fernández, DNI 20.999.888.",
  },
  {
    label: "Paciente homónimo (aclaración)",
    text: "Hola, soy Gómez y quiero cancelar el turno que tengo mañana.",
  },
  {
    label: "Reembolso alto (aprobación)",
    text: "Me cobraron mal la resonancia, quiero que me devuelvan los $180.000. Soy Lucía Ortiz, DNI 39.222.777.",
  },
  {
    label: "Consulta clínica (no responde)",
    text: "Tengo un dolor en el pecho desde ayer, ¿es algo grave? ¿qué me conviene tomar?",
  },
];
      ...
      <section className="flex min-h-[70vh] flex-1 flex-col rounded-xl border border-neutral-200 bg-white shadow-sm">
        <header className="border-b border-neutral-200 px-4 py-3">
          ...
          <p className="text-xs text-neutral-500">
            DurableAgent (Workflow DevKit) · pausa para pedir aprobación humana por
            Slack o acá al lado
          </p>
        </header>
        ...
        <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
          {messages.length === 0 && (
            <div className="space-y-3">
              <p className="text-sm text-neutral-500">
                Probá un escenario o escribí un mensaje como si fueras el paciente:
              </p>
          ...
          {busy && (
            <p className="text-xs text-neutral-400">el agente está trabajando…</p>
          )}
          {error && (
            <p className="text-xs text-red-600">
              Error: {error.message}. ¿Está seteada <code>ANTHROPIC_API_KEY</code>?
            </p>
          )}
        </div>
        ...
        >
          <input
            ...
            placeholder="Escribí como paciente…"
...
function ToolChip({
  ...
  const waiting =
    (name === "requestHumanApproval" || name === "askHumanInput") &&
  ...
  return (
    <aside className="flex w-full flex-col gap-3 rounded-xl border border-neutral-200 bg-white p-4 shadow-sm md:w-96">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold">Pendientes de un humano</h2>
        <span
      ...
      {data.pending.length === 0 && (
        <p className="text-xs text-neutral-400">
          Nada pendiente. Cuando el agente necesite aprobación o una aclaración,
          aparece acá (y en Slack si está configurado).
        </p>
      )}

      {data.pending.map((req) => (
        <RequestCard key={req.token} req={req} onResolved={refresh} />
      ))}
    </aside>
  );
}
  ...
  async function send(body: Record<string, unknown>) {
    ...
    try {
      await fetch("/api/approvals", {
        ...
        body: JSON.stringify({ token: req.token, respondedBy: who || undefined, ...body }),
      });
      onResolved();
    } finally {
      setSending(false);
  ...
  return (
    <div className="space-y-2 rounded-lg border border-neutral-200 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {req.riskLevel && (
          <span className={`rounded px-1.5 py-0.5 text-[10px] ${riskColor}`}>
            {req.riskLevel}
          </span>
        )}
      </div>
      {req.patientName && (
        <p className="text-[11px] text-neutral-500">👤 {req.patientName}</p>
      )}
